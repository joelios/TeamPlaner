...
@frappe.whitelist()
def update_this_user_in_events(user):
    events = frappe.db.sql("""SELECT `name` FROM `tabTP Event` WHERE `name` IN (
                                    SELECT `parent` FROM `tabTP Event Teilnehmer` WHERE `mitglied` = '{user}')""".format(user=user), as_dict=True)
    print("{qty} events".format(qty=len(events)))
    for event in events:
        print("fetch event")
        event = frappe.get_doc("TP Event", event.name)
...
def add_all_user_to_all_events():
    ...
    for user in users:
        print("check user")
        events = frappe.db.sql("""SELECT `name` FROM `tabTP Event`
                                  WHERE `name` NOT IN (
                                    SELECT `parent` FROM `tabTP Event Teilnehmer` WHERE `mitglied` = '{user}')
                                  AND `event_date` > CURDATE()""".format(user=user.name), as_dict=True)
        for event in events:
            print("update event")
            event = frappe.get_doc("TP Event", event.name)
...
@frappe.whitelist()
def add_this_user_to_all_events(user):
    events = frappe.db.sql("""SELECT `name` FROM `tabTP Event`
                            WHERE `name` NOT IN (
                                SELECT `parent` FROM `tabTP Event Teilnehmer` WHERE `mitglied` = '{user}')
                            AND `event_date` > CURDATE()""".format(user=user), as_dict=True)
    for event in events:
        print("update event")
        event = frappe.get_doc("TP Event", event.name)
        row = event.append('teilnehmer', {})
        mitglied = frappe.get_doc("Mitglied", user)
        row.mitglied = mitglied.name
        row.status = "Anwesend"
        row.user = mitglied.user_id
        row.bild = mitglied.bild
